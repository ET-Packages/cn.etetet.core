...
namespace ET
{
    ...
    public class WService: AService
    {
        ...
        public WService(IPEndPoint ipEndPoint)
        {
            ...
            StartAccept(ipEndPoint).NoContext();
        }
        ...
        private async ETTask StartAccept(IPEndPoint ipEndPoint)
        {
            string prefix = $"http://{ipEndPoint}/";
            try
            {
            ...
            catch (HttpListenerException e)
            {
                if (e.ErrorCode == 5)
                {
                    throw new Exception($"CMD管理员中输入: netsh http add urlacl url=http://*:8080/ user=Everyone   {prefix}", e);
                }
